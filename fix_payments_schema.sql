-- Fix for Order Payments Table
-- This script ensures the order_payments table exists and has correct RLS policies.

-- 1. Create Order Payments Table (if not exists)
...
DROP POLICY IF EXISTS "Admins can manage payments" ON public.order_payments;
...
-- Buyers: View OWN payments (linked via orders)
DROP POLICY IF EXISTS "Buyers can view own payments" ON public.order_payments;
